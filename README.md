...
`agent/spec-divisor-finder-agent.md` の指示で `candidate.py` に `find_divisor(n)` を実装し、`python eval/oracle.py --candidate candidate` で採点。candidate が無くても `reference` で全工程を再現できます。
...
## ファイル構成
- `agent/…md` … エージェント定義／`eval/oracle.py` … 仕様オラクル（`--selftest` 内蔵）
